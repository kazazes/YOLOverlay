...
class Settings: ObservableObject {
  ...
  private func loadModelMetadata() {
      ...
      let model = try? MLModel(contentsOf: modelURL)
    {
      ...
      if let description = model.modelDescription.metadata[
        MLModelMetadataKey(rawValue: "MLModelDescriptionKey")] as? String
      {
        modelDescription = description
      } else {
        modelDescription = isSegmentation ? "YOLO segmentation model" : "YOLO detection model"
      }
      ...
      modelMetadata = metadata.joined(separator: "\n")

      // Generate colors for classes if needed
      for className in modelClasses {
        if classColors[className] == nil {
          classColors[className] = Settings.availableColors.randomElement() ?? "red"
        }
      }
      ...
      modelDescription = "Failed to load model"
      modelMetadata = "Error: Model could not be loaded"
      modelClasses = []
